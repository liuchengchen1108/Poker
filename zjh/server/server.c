...
int main(int agrc,char **argv)
{
    ...
    while(1)
    {
        ...
        {
            if (FD_ISSET(0, &ser_fdset)) //标准输入(标准输入输出是0)是否存在于ser_fdset集合中（也就是说，检测到输入时，做如下事情）
            {
			...
			printf("%s\r\n",input_message);
                 */    
			if (write(client_fds[i], input_message, strlen(input_message)) < 0) 
			{
			    printf("write error \r\n ");
			  
			} else {
			
			    printf("client fd = %d ,msg = %s\r\n", client_fds[i], input_message);
			}
            ...
            if (FD_ISSET(ser_souck_fd, &ser_fdset))
            {
                ...
                {
                    int flags = -1;
                    //一个客户端到来分配一个fd，CLI_NUM = 6，则最多只能有三个客户端，超过7以后跳出for循环，flags重新被赋值为-1
                    for(i = 0; i < CLI_NUM; i++)
                    {
                        if(client_fds[i] == 0)
                        {
